...
const filters = reactive<{ year: number | null; office_id: number | null }>({
  year: savedSearch?.year ?? getCurrentYear(), // 還原上次搜尋年度；無儲存才用當年度
  office_id: savedSearch?.office_id ?? getUserOfficeId(), // 還原上次搜尋管理處；無儲存才用使用者所屬
})
...
const availableOfficeOptions = computed(() => {
  ...
  if (role === 'admin') {
    // Admin 看到重新定義的列表
    return officeOptions.value.filter(opt => adminAllowedIds.includes(opt.value))
  }
  
  // User 只能看到自己的管理處
  return officeOptions.value.filter(opt => opt.value === currentUserOfficeId.value)
})
