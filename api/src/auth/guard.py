...
from src.auth.users import AUTH_SOURCE_SSO, check_password_expired
...
async def require_full_auth(
    ...
    042（FR-014）：由入口平台 SSO 進入的工作階段略過帳號鎖定與密碼過期兩項檢查。
    兩者保護的都是 AERC 本地密碼，而 SSO 路徑從不驗證本地密碼——身分由入口平台負責
    （客戶 2026-09-08 確認）。帳號啟用狀態仍由 get_current_user 檢查，不在豁免之列。
    在此單點判別，139 個呼叫點不需改動。
    """
    if current_user.auth_source == AUTH_SOURCE_SSO:
        return current_user
