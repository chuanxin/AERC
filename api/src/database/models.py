...
class Users(models.Model):
    ...
    office = fields.ForeignKeyField(
        "models.Offices", related_name="user", null=True, description="所屬單位/管理處"
    )
    department = fields.JSONField(
        null=True,
    ...
    role = fields.CharField(
        max_length=50, default="user", description="角色: admin, manager, user 等"
    )
    permissions = fields.JSONField(
        null=True,
        description="使用者權限設定（JSONB）：{mode: 'default'|'scoped'|'custom', scope: {...}, custom: {...}}",
    )
    last_login = fields.DatetimeField(null=True, description="最後登入時間")

    # 密碼政策相關欄位
    password_changed_at = fields.DatetimeField(
        null=True, description="密碼最後更改時間"
    )
...
class AuthToken(models.Model):
    ...
    user = fields.ForeignKeyField(
        "models.Users", related_name="auth_tokens", description="所屬用戶"
    )
    token_type = fields.CharEnumField(AuthTokenType, description="Token 類型")
    token = fields.CharField(
        max_length=128, unique=True, description="Token 值（UUID）"
    )
    ...
    user_agent = fields.CharField(
        max_length=255, null=True, description="請求 User-Agent"
    )
...
class UserRegistration(models.Model):
    ...
    user = fields.OneToOneField(
        "models.Users", related_name="registration", description="申請的使用者帳號"
    )
    ...
    # 審核流程
    status = fields.CharEnumField(
        RegistrationStatus, default=RegistrationStatus.PENDING, description="申請狀態"
    )
...
class PasswordHistory(models.Model):
    ...
    id = fields.IntField(pk=True)
    user = fields.ForeignKeyField(
        "models.Users", related_name="password_history", description="使用者"
    )
    ...
    changed_by_ip = fields.CharField(
        max_length=45, null=True, description="變更來源 IP"
    )
    user_agent = fields.CharField(max_length=255, null=True, description="User Agent")
    change_method = fields.CharField(
        max_length=50,
        null=True,
        description="變更方式: password_reset, user_change, admin_reset",
    )
...
class Grants(models.Model):
    ...
    active_version = fields.ForeignKeyField(
        "models.GrantVersions",
        related_name="active_grant",
        null=True,
        description="目前現行的版本ID",
    )
    ...
    status = fields.CharField(
        max_length=20,
        default="draft",
        description="案件狀態: 0:完成申請人資料, 1:完成土地資料, 2:完成灌溉調控設施, 3:完成田間管路, 4:完成現場勘查, 5:完成補助申請資料, 6:完成結案申報, 7:完成測試合格的時間, 8:完成撥款作業, 9:完成撥款, 99:駁回申請",
    )
    ...
    bulletin = fields.CharField(
        max_length=20,
        null=True,
        description="公告狀態: 0:已受理, 1:審查中, 2:審查通過 3:結案流程 4:撥款作業 5:撥款完成",
    )
    bulletin_sys = fields.CharField(
        max_length=20,
        null=True,
        description="公告狀態(系統): 0:申請人資料, 1:現場勘查, 2:補助申請資料 3:結案申報 4:測試合格的時間 5:",
    )
    ...
    async def generate_sn(cls, year: int, office_id: int) -> int:
        ...
        last_grant = (
            await cls.filter(year=year, office_id=office_id).order_by("-sn").first()
        )
...
class GrantAttachments(models.Model):
    ...
    id = fields.IntField(pk=True)
    grant = fields.ForeignKeyField(
        "models.Grants", related_name="attachments", description="所屬補助申請案件"
    )
    ...
    step = fields.IntField(
        description="申請步驟編號 (5:現場勘查, 6:補助申請, 7:結案申報, 8:測試合格)"
    )
    category = fields.CharField(
        max_length=20, description="附件分類 (如:施工前照片、施工後照片、收據等)"
    )

    # 檔案資訊（關鍵設計）
    original_filename = fields.CharField(
        max_length=255, description="使用者上傳的原始檔名"
    )
    internal_filename = fields.CharField(
        max_length=255, description="系統內部儲存檔名 (UUID格式)"
    )
    ...
    status = fields.CharField(
        max_length=20,
        default="active",
        description="附件狀態 (active:有效, deleted:已刪除)",
    )

    # 關聯性（Step 7 前後對比用）
    related_attachment = fields.ForeignKeyField(
        "models.GrantAttachments", null=True, description="關聯附件ID (用於前後對比)"
    )

    # 審計欄位
    uploaded_at = fields.DatetimeField(auto_now_add=True, description="上傳時間")
    uploaded_by = fields.ForeignKeyField(
        "models.Users", related_name="uploaded_attachments", description="上傳人員"
    )
...
class GrantComments(models.Model):
    ...
    id = fields.IntField(pk=True)
    grant = fields.ForeignKeyField(
        "models.Grants", related_name="comments", description="所屬案件"
    )
    user = fields.ForeignKeyField(
        "models.Users", related_name="grant_comments", description="評論者"
    )
...
class GrantHistory(models.Model):
    ...
    id = fields.IntField(pk=True)
    grant = fields.ForeignKeyField(
        "models.Grants", related_name="history", description="所屬案件"
    )
...
class CropNames(models.Model):
    ...
    category = fields.ForeignKeyField(
        "models.CropCategories", related_name="crop_name", description="所屬作物類別"
    )
...
class SubsidyAnnualBudget(models.Model):
    ...
    approved_area = fields.DecimalField(
        max_digits=10, decimal_places=4, default=0, description="核定執行面積（公頃）"
    )
...
class Towns(models.Model):
    ...
    id = fields.IntField(pk=True)
    county = fields.ForeignKeyField(
        "models.Counties", related_name="town", description="所屬縣市"
    )
    ...
    indigenous_type = fields.CharField(
        max_length=10, null=True, description="原民區域類型(1:山地鄉 2:平地鄉)"
    )
...
class Villages(models.Model):
    ...
    town = fields.ForeignKeyField(
        "models.Towns", related_name="village", description="所屬鄉鎮市區"
    )
...
class Sections(models.Model):
    ...
    id = fields.IntField(pk=True)
    town = fields.ForeignKeyField(
        "models.Towns", related_name="section", description="所屬鄉鎮市區"
    )
...
class PipeFittings(models.Model):
    ...
    pomno = fields.IntField(pk=True, description="管件代碼（技術主鍵）")
    item_sn = fields.CharField(
        max_length=50, null=True, description="業務料號/品號（與pomno分離）"
    )
    name = fields.CharField(max_length=50, unique=False, description="管件名稱")
    material = fields.ForeignKeyField(
        "models.PFMaterials", related_name="pf_material", description="所屬管件材質"
    )
    module = fields.ForeignKeyField(
        "models.PFModules", related_name="pf_module", description="所屬管件功能類型"
    )
    diameter1 = fields.ForeignKeyField(
        "models.PFDiameters",
        related_name="pf_diameter1",
        null=True,
        description="所屬管徑1",
    )
    diameter2 = fields.ForeignKeyField(
        "models.PFDiameters",
        related_name="pf_diameter2",
        null=True,
        description="所屬管徑2",
    )
    diameter3 = fields.ForeignKeyField(
        "models.PFDiameters",
        related_name="pf_diameter3",
        null=True,
        description="所屬管徑3",
    )
    unit = fields.CharField(max_length=10, null=True, description="管件計量單位")
    description = fields.CharField(max_length=255, default="", description="管件描述")
    office = fields.ForeignKeyField(
        "models.Offices",
        related_name="pipe_fittings",
        null=True,
        description="所屬單位/管理處",
    )
    length = fields.FloatField(null=True, description="管件長度")
    compatibility_group = fields.JSONField(null=True, description="相容性分組")
    typical_location = fields.CharField(
        max_length=255, null=True, description="典型使用位置"
    )
    is_active = fields.BooleanField(default=True, description="是否啟用")
    ...
    created_by = fields.ForeignKeyField(
        "models.Users",
        related_name="created_pipe_fittings",
        description="建立人帳號",
        null=True,
        on_delete=fields.CASCADE,
    )
    modified_by = fields.ForeignKeyField(
        "models.Users",
        related_name="modified_pipe_fittings",
        description="修改人帳號",
        null=True,
        on_delete=fields.SET_NULL,
    )
...
class IrrigationTypes(models.Model):
    ...
    description = fields.CharField(
        max_length=255, null=True, description="灌溉類型描述"
    )
    is_active = fields.BooleanField(default=True, description="是否啟用")
    parent = fields.ForeignKeyField(
        "models.IrrigationTypes",
...
class PFAnnualPrices(models.Model):
    ...
    id = fields.IntField(pk=True)
    pipe_fitting = fields.ForeignKeyField(
        "models.PipeFittings", related_name="annual_prices", description="所屬管件"
    )
    office = fields.ForeignKeyField(
        "models.Offices",
        related_name="pf_annual_prices",
        null=True,
        description="所屬單位/管理處",
    )
    is_active = fields.BooleanField(default=True, description="是否啟用")
    ...
    created_by = fields.ForeignKeyField(
        "models.Users",
        related_name="pf_annual_prices_created",
        description="建立人帳號",
        null=True,
        on_delete=fields.CASCADE,
    )
    modified_by = fields.ForeignKeyField(
        "models.Users",
        related_name="pf_annual_prices_modified",
        description="修改人帳號",
        null=True,
        on_delete=fields.SET_NULL,
    )
...
class SubsidySettings(models.Model):
    ...
    id = fields.IntField(pk=True)
    irrigation_type = fields.ForeignKeyField(
        "models.IrrigationTypes",
        related_name="subsidy_settings",
        description="所屬灌溉類型",
    )
    facility_type = fields.ForeignKeyField(
        "models.FacilityTypes",
        related_name="subsidy_settings",
        description="所屬設施類型",
    )
    year = fields.IntField(description="年度")
    founding_source = fields.ForeignKeyField(
    ...
    created_by = fields.ForeignKeyField(
        "models.Users",
        related_name="created_subsidy_settings",
        description="建立人帳號",
        null=True,
        on_delete=fields.CASCADE,
    )
    modified_by = fields.ForeignKeyField(
        "models.Users",
        related_name="modified_subsidy_settings",
        description="修改人帳號",
        null=True,
        on_delete=fields.SET_NULL,
    )
...
class FacilityTypes(models.Model):
    ...
    code = fields.CharField(
        max_length=10, unique=True, null=True, description="設施類型代碼"
    )
    description = fields.CharField(
        max_length=255, null=True, description="設施類型描述"
    )
    is_active = fields.BooleanField(default=True, description="是否啟用")
    created_at = fields.DatetimeField(auto_now_add=True, description="建立時間")
    modified_at = fields.DatetimeField(auto_now=True, description="修改時間")
...
class SubsidyPolicies(models.Model):
    ...
    id = fields.IntField(pk=True)
    year = fields.IntField(description="年度")
    funding_source = fields.ForeignKeyField(
        "models.FundingSources",
        related_name="subsidy_policies",
        description="所屬補助來源",
    )
    ...
    created_by = fields.ForeignKeyField(
        "models.Users",
        related_name="created_subsidy_policies",
        description="建立人帳號",
        null=True,
        on_delete=fields.CASCADE,
    )
    modified_by = fields.ForeignKeyField(
        "models.Users",
        related_name="modified_subsidy_policies",
        description="修改人帳號",
        null=True,
        on_delete=fields.SET_NULL,
    )
...
class IrrigationSubsidies(models.Model):
    ...
    id = fields.IntField(pk=True)
    subsidy_policy = fields.ForeignKeyField(
        "models.SubsidyPolicies",
        related_name="irrigation_subsidies",
        description="所屬補助政策",
    )
    irrigation_type = fields.ForeignKeyField(
        "models.IrrigationTypes",
...
class WaterSources(models.Model):
    ...
    code = fields.CharField(
        max_length=10, unique=True, null=True, description="水源代碼"
    )
...
class PowerEquipments(models.Model):
    ...
    code = fields.CharField(
        max_length=10, unique=True, null=True, description="動力設備代碼"
    )
    description = fields.CharField(
        max_length=255, null=True, description="動力設備描述"
    )
    is_active = fields.BooleanField(default=True, description="是否啟用")
...
class RegulationEquipments(models.Model):
    ...
    code = fields.CharField(
        max_length=10, unique=True, null=True, description="調控設備代碼"
    )
    description = fields.CharField(
        max_length=255, null=True, description="調控設備描述"
    )
    is_active = fields.BooleanField(default=True, description="是否啟用")
...
class GrantVersions(models.Model):
    ...
    id = fields.IntField(pk=True)
    grant = fields.ForeignKeyField(
        "models.Grants", related_name="versions", description="所屬補助申請"
    )
    version = fields.IntField(description="版本資訊")
    all_steps_data = fields.JSONField(description="所有步驟的資料(JSON格式)")
    all_steps_data_hash = fields.CharField(
        max_length=64, description="所有步驟資料的Hash值，用於檢查版本變更", null=True
    )
    data_schema_version = fields.CharEnumField(
        DataSchemaVersions, default=DataSchemaVersions.V1_0, description="資料結構版本"
    )
...
class GrantPapers(models.Model):
    ...
    id = fields.IntField(pk=True)
    version = fields.ForeignKeyField(
        "models.GrantVersions", related_name="reports", description="所屬補助申請版本"
    )
    document_type = fields.CharField(max_length=50, description="文件類型")
    document_data = fields.JSONField(description="文件內容")
    data_hash = fields.CharField(
        max_length=64, description="文件內容的Hash值，用於檢查變更", null=True
    )
...
class QualificationQuery(models.Model):
    ...
    location_data = fields.JSONField(
        description="地區查詢參數: {county, town, section?, landNumber?}"
    )

    # 查詢選項
    query_options = fields.JSONField(
        null=True, description="查詢選項: {years, includeStatistics}"
    )
    ...
    query_hash = fields.CharField(
        max_length=64, null=True, description="查詢參數雜湊值(用於快取)"
    )
